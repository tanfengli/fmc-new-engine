 ...
 * 	组织架构类型：orgTypeArr
 * 	组织架构人员列表：elementArray
 * 	组织架构人员ids：elementIds
 *  组织架构人员names：elementNames
 * 
 * 确认回调函数：
...
define([ 'knockout','AjaxUtil','MsgBox'], function(ko,AjaxUtil,MsgBox) {
	function groupMemberTree(params) {
		...
		//组织架构树Id
		self.treeId = 'elementTree';
		//常用地址本树Id(群组、通用岗位、角色线)
		self.addressId = 'addressTree';
		//常用地址本树Id(群组)
		self.groupId = 'groupTree';
		...
		self.orgTypes3=[ "4", "8"];
		//操作类型列表
		self.orgTypeList = ko.observableArray([]);
		...
		//组织类型
		self.elementSelected = ko.observableArray([]);
		//查询条件
		...
		//模态框初始化加载
		self.initModal = $('#memberSelectWindow').on('show.bs.modal', function () {
			self.rightElement([]);
			self.filterName("")
			self.initModal();
		...
		self.initModalData = $('#memberSelectWindow').on('shown.bs.modal', function () {
			self.initRightElements();
		})
		...
		self.initModal = function() {
			...
			//设置组织架构类型
			var setOrgType = function(val){
				if(val&&val.length){
				  self.orgTypeList.removeAll();
				  self.elementSelected.removeAll();
				  ko.utils.arrayPushAll(self.orgTypeList,val);
				  ko.utils.arrayPushAll(self.elementSelected,val);
				}
			}
			
			switch(self.pattern()){
			case ('handler'):
				setOrgType(self.orgTypes2);
				break;
			case ('group'):
				setOrgType(self.orgTypes1);
				break;
			case ('privileger'):
				setOrgType(self.orgTypes3);
				break;
			}
			
			if(ko.toJS(params.data).orgTypeArr!=undefined){
				setOrgType(ko.toJS(params.data).orgTypeArr)
			}
		};
		
		//初始化已选列表
		self.initRightElements = function(){
			//监控已选列表
			self.rightElement.removeAll();
			var elementArray = ko.toJS(params.data).elementArray;
			var elementIds = ko.toJS(params.data).elementIds;
			var elementNames = ko.toJS(params.data).elementNames;
			if(elementArray!=undefined){
				for (i=0;i<elementArray.length;i++){
					self.rightElement.push(elementArray[i]);
				}
			}else if(elementIds!=undefined&&elementNames!=undefined){
				var ids = elementIds.split(';');
				var names = elementNames.split(';');
				for (i=0;i<ids.length;i++){
					if(null!=ids[i]&&ids[i]!="")
						self.rightElement.push({fdId : ids[i],fdName : names[i]});
				}
			}
		}
		...
		self.groupNodeChange = function(node){
			...
			if(!node.fdIsLeaf==1&&self.pattern()=='group'){
				AjaxUtil.call({
...
//		}
		...
		self.leftSelectChange = function(a){
		}
		
		//上下选中更新描述
		self.leftChange = function(){
		...
		// 组织架构树model
		self.elementTree = new ko.zTreeView('elementTree', {
